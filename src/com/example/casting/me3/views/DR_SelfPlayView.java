...
public class DR_SelfPlayView implements OnClickListener,OnItemClickListener{
	...
	boolean edit = false;
	public DR_SelfPlayView(Context ctx , boolean isEdit){
		mContext = ctx;
		edit = isEdit;
		initView(ctx);
	}
	
	public DR_SelfPlayView(Context ctx,List<GalleryBean> beans) {
		mContext = ctx;
		gallerylist.clear();
	...
	public void initView(Context ctx) {
		...
		if(edit){
			txt_add.setText("  添加个人展示");
		}else{
			txt_add.setText("  查看更多个人展示");
