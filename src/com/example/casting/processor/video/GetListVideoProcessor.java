package com.example.casting.processor.video;
...
public class GetListVideoProcessor extends BaseProcessor {

	@Override
	public String getService() {
		// TODO Auto-generated method stub
		return ProcessorID.service_video;
	}
		...
		try {
			...
			for( int i = 0 ; i < size ; i++){
				if( !array.isNull(i)){
					...
					VideoBean bean = map2Bean(new CastMap(json));
					beans.add(bean);
				}
			}
	...
	protected VideoBean map2Bean(CastMap map) {
		if( map!=null){
			VideoBean bean = new VideoBean();
			bean.setUrl(map.get("video_url"));
