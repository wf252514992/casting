...
public class WinnerGetListProcessor extends WinnerBaseProcessor{
	...
	public String getMethod() {
		...
		return ProcessorID.method_winner_getList;
	}
	...
	public ResultBean json2Bean(String returnstr) {
		try {
			...
			List<WinnerBean> beans = new ArrayList<WinnerBean>();
			for( int i = 0 ; i < array.length() ; i++){
				if( !array.isNull(i)){
					JSONObject json = (JSONObject) array.get(i);
					WinnerBean bean = map2bean(new CastMap(json));
					beans.add(bean);
				}
			}
