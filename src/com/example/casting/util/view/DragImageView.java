...
public class DragImageView extends ImageView {
	...
	private double down_time,up_time;
...
/**是否是单点触摸*/
	...
	onMyImageViewClickListener clickListener;
	/** 构造方法 **/
	public DragImageView(Context context) {
		super(context);
		this.context = context;
	}
	public void setOnImageViewClickListener(onMyImageViewClickListener clickListener)
	{
	...
	public void setImageBitmap(Bitmap bm) {
		Bitmap bmp = PicTool.getBitmap(bm, window_width, window_height);
//		BitmapDrawable dra = new BitmapDrawable(bmp);
		super.setImageBitmap(bmp);
//		super.setImageDrawable(dra);
//		Bitmap bmp = PicTool.getBitmap(bm, window_width, window_height);
	
		if (bmp==null)return;
		/** 获取图片宽高 **/
		bitmap_W = bmp.getWidth();//480
		bitmap_H = bmp.getHeight();//692
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if((up_time-down_time)/1000 <= 0.1 && isSinglePoint)
			{
				if(clickListener != null)
				{
					clickListener.onImageViewClick();
				}
			}
			mode = MODE.NONE;
			break;
		...
		if (mode == MODE.DRAG) {
			...
			if (isControl_H) {
				...
				if (right <= window_width) {
					left = window_width - this.getWidth();
					right = window_width;
				}
			} else {
				left = this.getLeft();
			...
			if (isControl_V) {
				...
				if (bottom <= window_height) {
					top = window_height - this.getHeight();
	...
	void setScale(float scale) {
		...
		if (scale > 1 && this.getWidth() <= MAX_W) {
			 ...
			 */
			if (current_Top <= 0 && current_Bottom >= window_height) {
		//		Log.e("jj", "屏幕高度=" + this.getHeight());
				isControl_V = true;// 开启垂直监控
			} else {
				isControl_V = false;
			}
			if (current_Left <= 0 && current_Right >= window_width) {
		...
		// 缩小
			...
			if (isControl_V && current_Top > 0) {
				...
				if (current_Bottom < window_height) {
					current_Bottom = window_height;
					isControl_V = false;// 关闭垂直监听
				}
			}
			// 下边越界
			if (isControl_V && current_Bottom < window_height) {
				current_Bottom = window_height;
			...
			if (isControl_H && current_Left >= 0) {
				...
				if (current_Right <= window_width) {
					current_Right = window_width;
					isControl_H = false;// 关闭
				}
			}
			// 右边越界
			if (isControl_H && current_Right <= window_width) {
				current_Right = window_width;
	...
	public void doScaleAnim() {
		myAsyncTask = new MyAsyncTask(window_width, this.getWidth(),
				this.getHeight());
	...
	class MyAsyncTask extends AsyncTask<Void, Integer, Void> {
		private int window_width, current_Width, current_Height;
		...
		public MyAsyncTask(int window_width, int current_Width, int current_Height) {
			super();
			this.window_width = window_width;
		...
		protected Void doInBackground(Void... params) {
			System.out.println("--start_Top----"+start_Top+"---start_Right----"+start_Right
					+"---start_Bottom---"+start_Bottom +"----start_Left---"+start_Left);
			while (current_Width <= window_width) {
		...
		protected void onProgressUpdate(final Integer... values) {
			...
			mActivity.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					System.out.println("----setFrame---"+values[0]+"---"+values[1]+"---"+values[2]+"---"+values[3]);
				}
			});
	...
	public interface onMyImageViewClickListener
	{
